...
function nys_legislation_explorer_general_search() {
  ...
  $vars['issue_terms'] = get_issue_codes();
...
function get_bill_search_query($search) {
  $fq_params = array(
    ...
    'sm_field_ol_latest_status_commit' => (!empty($search['bill_committee'])) ? '"' . $search['bill_committee'] . '"' : '',
