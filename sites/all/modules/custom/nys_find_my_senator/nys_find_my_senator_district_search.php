...
function nys_legislation_explorer_district_search() {
  ...
  if (isset($_GET['search']) && $_GET['search'] == 'true') {
    ...
    foreach (array('addr1', 'city', 'zip5') as $val) {
      // Remove any encoding from the value.
      $new_val = urldecode(isset($_GET[$val]) ? $_GET[$val] : '');
      // Remove suspect characters, and entity-ize the rest.
      $api_data[$val] = str_replace(['<','>'], '', $new_val);
