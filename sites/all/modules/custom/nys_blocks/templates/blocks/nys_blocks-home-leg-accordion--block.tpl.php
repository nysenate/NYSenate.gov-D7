...
<?php $session = nodequeue_load_nodes(4); ?>
<?php
$view1 = views_get_view('upcoming_legislation_calendar');
if(!empty($view1)) {
  $view1->set_display('sess_floor_calendar');
  $view1->set_arguments(array($session[0]->nid));
  $view1->pre_execute();
  $view1->execute();
}
if(isset($session[0]->nid)) $session_loaded = node_load($session[0]->nid);
...
<div class="nys-accordion--header">
  ...
  <a href="/<?php echo drupal_get_path_alias('node/'.$session[0]->nid)?>" class="upcoming-events-link c-container--link">Go to Session Event</a>
</div>
...
if(!empty($view2)) {
  ...
  $view2->set_arguments(array($session[0]->nid));
  $view2->pre_execute();
  $view2->execute();
}
...
if(!empty($view3)) {
  ...
  $view3->pre_execute();
  $view3->set_arguments(array($session[0]->nid));
  $view3->execute();
}
