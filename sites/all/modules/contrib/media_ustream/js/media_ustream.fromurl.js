...
/**
 *  @file
 *  Create the 'UStream' tab for the WYSIWYG plugins.
 */

(function ($) {
  namespace('media.browser.plugin');

  Drupal.media.browser.plugin.ustream_library = function(mediaBrowser, options) {

    return {
      mediaFiles: [],
      init: function() {
        tabset = mediaBrowser.getTabset();
        tabset.tabs('add', '#ustream_library', 'UStream');
        var that = this;
        mediaBrowser.listen('tabs.show', function (e, id) {
          if (id == 'ustream_library') {
  ...
  Drupal.media.browser.register('ustream_library', Drupal.media.browser.plugin.ustream_library);
})(jQuery);
