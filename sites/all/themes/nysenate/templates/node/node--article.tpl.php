...
<article id="node-<?php print $node->nid; ?>"
  ...
  <?php if ($page): ?>
      <header class="c-news--head">
          ...
          <div class="c-news--head-meta">
            <?php if (!empty($content['field_senator'][0]['#markup'])): ?>
                <p class="c-news--author">
                  <?php print $content['field_senator'][0]['#markup']; ?>
                </p>
            <?php elseif (!empty($content['field_article_author'][0]['#markup'])): ?>
                <p class="c-news--author">
            ...
            <?php if (isset($content['field_committee']['#items'])): ?>
                <ul class="nys-associated-topics">
                    COMMITTEE:
                  <?php for ($i = 0; $i < count($content['field_committee']['#items']); $i++) :
                    if (!empty($content['field_committee'][$i])):
                      print '<li>' . render($content['field_committee'][$i]) . '</li>';
                    endif;
                  endfor; ?>
                </ul>
            <?php endif; ?>
  ...
  <?php print render($title_suffix); ?>
      ...
      <?php if (!empty($node->field_image_main)): ?>
          <div class="c-block c-block--img">
            ...
            <?php if (!empty($node->field_image_main[LANGUAGE_NONE][0]['title'])): ?>
                <p
                        class="c-img--caption"><?php print
                    $node->field_image_main[LANGUAGE_NONE][0]['title']; ?></p>
            <?php endif; ?>
          </div>
      <?php endif; ?>

      <?php if (!empty($content['field_subtitle'][0]['#markup'])): ?>
    ...
    <?php endif; ?>
      ...
      <?php if (!empty($content['field_bills'])): ?>
          <aside>
