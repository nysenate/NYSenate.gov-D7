...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?>"<?php print $attributes; ?>>
    <header class="c-news--head">
        ...
        <?php if (!empty($content['field_date'][0]['#markup'])): ?>
            <p class="c-news--pub-date">
                <?php print $content['field_date'][0]['#markup']; ?>
            </p>
        <?php endif; ?>
        <?php if (isset($content['field_issues']['#items'])): ?>
	...
	<?php if(isset($content['field_image_main'][0]['#item']['uri'])): ?>
		<div class="c-initiative--featured-image">
			...
			<?php if(!empty($content['field_image_main'][0]['#item']['title'])): ?>
				<figcaption class="c-img--caption"><?php echo $content['field_image_main'][0]['#item']['title']; ?></figcaption>
			<?php endif; ?>
		</div>
	<?php endif; ?>
    ...
    <?php if(isset($content['field_profile_military_branch'])){ ?>
        <div class="c-initiative--section-title">
            <?php echo ($content['field_profile_military_branch'][0]['#markup']);
    }
            if(isset($content['field_subheading'])){
                echo (' | ' . $content['field_subheading'][0]['#markup']);
    }  ?>
        </div>
    <?php  ?>
    ...
    <?php if(isset($content['field_pg_video'])): ?>
        <div class="c-initiative c-container--header__top-border">
            <h2 class="c-container--title">Honoree Video</h2>
        </div>
        <div class="c-initiative--video">
            <?php print render($content['field_pg_video']); ?>
        </div>
    <?php endif; ?>
