...
<article id="node-<?php print $node->nid; ?>"
         class="event_detail_page <?php print $classes; ?>"<?php print $attributes; ?>>

    <h1 class="nys-event-title"><?php print $title; ?></h1>
    <div class="c-event-block c-event-block--featured<?php if ($node->field_image_main) {
      print "-image";
    } ?>">
      ...
      <?php endif; ?>
      <?php print theme('nysenate_event_date', [
      ...
      <?php if($content['field_event_place']['#items'][0]['value'] == 'online'): ?>
          <div class="c-event-location"><span
                      class="icon-before__circle-pin"></span><a href="<?php print $content['field_event_online_link']['#items'][0]['url']; ?>">Online Event</a>

        <?php if(!empty($content['field_meeting_location']['#items'][0]['value'])): ?>
              <br><?php print $content['field_meeting_location']['#items'][0]['value'] ?>
        <?php endif; ?>
          </div>
      <?php elseif($content['field_event_place']['#items'][0]['value'] == 'teleconference'): ?>
        <div class="c-event-location"><span
                    class="icon-before__circle-pin"></span>Teleconference Event</div>

            <div class="c-event-location"><strong>TEL: </strong><?php print $content['field_teleconference_number']['#items'][0]['value']; ?>
                  <?php if(!empty($content['field_teleconference_id_number']['#items'][0]['value'])): ?>
                  <br><strong>ID: </strong><?php print $content['field_teleconference_id_number']['#items'][0]['value']; ?>
                  <?php endif; ?>
            <?php if(!empty($content['field_meeting_location']['#items'][0]['value'])): ?>
                <br><?php print $content['field_meeting_location']['#items'][0]['value'] ?>
            <?php endif; ?>
        ...
        <?php elseif(!empty($node->field_location[LANGUAGE_NONE][0]['name'])): ?>
              <a href="http://maps.google.com/?q=<?php echo $node->field_location[LANGUAGE_NONE][0]['street']; ?>+<?php echo $node->field_location[LANGUAGE_NONE][0]['city']; ?>%2C+<?php echo $node->field_location[LANGUAGE_NONE][0]['province']; ?>%2C+<?php echo $node->field_location[LANGUAGE_NONE][0]['postal_code']; ?>"
                 class="c-event-location" target="_blank"><span
                          class="icon-before__circle-pin"></span><?php print $node->field_location[LANGUAGE_NONE][0]['name']; ?>
              </a>
        <div class="c-event-address"><?php if(!empty($content['field_meeting_location']['#items'][0]['value'])): ?>
              <br><?php print $content['field_meeting_location']['#items'][0]['value'] ?>
          <?php endif; ?></div>
        <?php endif; ?>
        ...
        <div class="c-event-time"><?php print format_date($node->field_date[LANGUAGE_NONE][0]['value'], "custom", "g:i A"); ?><?php if (isset($node->field_date[LANGUAGE_NONE][0]['value2']) && ($node->field_date[LANGUAGE_NONE][0]['value2'] != $node->field_date[LANGUAGE_NONE][0]['value'])) {
            print '&nbsp-&nbsp' . format_date($node->field_date[LANGUAGE_NONE][0]['value2'], "custom", "g:i A");
          } ?></div>
