<div class="c-block c-block-search-result">
    ...
    <div class="l-col l-col-2-of-2">
        ...
        <div class="c-search-result--date">
          <?php if (!empty($term->field_senator[LANGUAGE_NONE][0]['target_id'])): ?>
            <?php print '<span>Senator </span>' . drupal_render($content['field_senator']); ?>
          <?php endif; ?>
          <?php if (!empty($term->field_chair[LANGUAGE_NONE][0]['target_id'])): ?>
            <?php print '<span>Chair </span>' . drupal_render($content['field_chair']); ?>
          <?php endif; ?>
        </div>
      <?php if (
        (isset($term->vocabulary_machine_name)
          && $term->vocabulary_machine_name == 'committees')
        && (isset($node->field_assemblymen_chair[LANGUAGE_NONE][0]['value'])
          && $node->field_assemblymen_chair[LANGUAGE_NONE][0]['value'] == 1)
      ): ?>
      ...
      <?php elseif (
        (isset($term->vocabulary_machine_name)
          && $term->vocabulary_machine_name == 'committees')
        && (isset($node->field_assemblymen_chair[LANGUAGE_NONE][0]['value'])
          && $node->field_assemblymen_chair[LANGUAGE_NONE][0]['value'] == 0)
      ): ?>
          <div class="c-search-result--follow">
